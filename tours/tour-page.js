...
const checkIcon = '<span class="item-icon check" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M4 12.5l5 5L20 6"/></svg></span>';
const crossIcon = '<span class="item-icon cross" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M6 6l12 12M18 6L6 18"/></svg></span>';
const includes = details.includes.map(item => `<li>${checkIcon}<span>${item}</span></li>`).join('');
const excludes = details.excludes.map(item => `<li>${crossIcon}<span>${item}</span></li>`).join('');
const durationIcon = '<span class="fact-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3.5 2"/></svg></span>';
const difficultyIcon = '<span class="fact-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M4 20V11l4-3 4 3 4-6 4 6v9"/></svg></span>';
const altitudeIcon = '<span class="fact-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M3 19l6-11 4 6.5L16 9l5 10z"/></svg></span>';
...
document.querySelector('#tour-page').innerHTML = `<section class="hero" style="background-image:url('../assets/images/${heroImage}')"><div><p class="eyebrow">${label}</p><h1>${name}</h1><p>Una experiencia auténtica por el Perú.</p></div></section><section class="content"><div><h2>Descubre ${name}</h2><p>${description}</p><div class="facts"><span>${durationIcon}<b>Duración</b><em>${duration}</em></span><span>${difficultyIcon}<b>Dificultad</b><em>${difficulty}</em></span><span>${altitudeIcon}<b>Altitud máxima</b><em>${altitude}</em></span></div><h2>Ruta de la experiencia</h2><div class="route">${route}</div><div class="guide-grid"><section><h3 class="good">${checkIcon}Incluye</h3><ul>${includes}</ul></section><section><h3 class="no">${crossIcon}No incluye</h3><ul>${excludes}</ul></section></div><h2>Recomendaciones</h2><p>Lleva ropa adecuada para el clima, agua, protector solar y consulta con nuestro equipo cualquier requerimiento especial antes de viajar. Para rutas de altura, considera uno o dos días de aclimatación en Cusco.</p></div><aside class="side"><p class="eyebrow">WAYNA INKA PERÚ AGENCY</p><h2>¿Quieres vivir esta experiencia?</h2><p>Consulta disponibilidad, itinerario y detalles para tu fecha de viaje.</p><a class="button" target="_blank" href="https://wa.me/${WHATSAPP}?text=${encodeURIComponent(`Hola, WAYNA INKA PERÚ AGENCY. Quiero información y disponibilidad para el tour ${name}. ¿Me pueden ayudar, por favor?`)}">Consultar por WhatsApp</a></aside></section>`;
